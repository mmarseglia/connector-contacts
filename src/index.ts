...
server.tool(
  ...
  "Create a new contact in the macOS address book. Only firstName is required; all other fields are optional.",
  {
    ...
    birthday: z.string().regex(/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/, "Birthday must be in YYYY-MM-DD format").refine((v) => { try { const d = new Date(v + "T00:00:00"); return d.toISOString().startsWith(v); } catch { return false; } }, { message: "Birthday is not a valid calendar date" }).optional().describe("Birthday in YYYY-MM-DD format"),
    phoneNumbers: z.array(z.string().min(1).regex(/^\+\d{2,}$/, "Phone number must be in E.164 format (e.g. +14155551234)")).optional().describe("Phone numbers (E.164 format, e.g. +14155551234)"),
    emailAddresses: z.array(z.string().min(1).regex(/^[^\s@]+@[^\s@]+\.[^\s@]+$/, "Invalid email address format")).optional().describe("Email addresses"),
    urlAddresses: z.array(z.string().min(1).regex(/^https?:\/\/\S+$/, "URL must start with http:// or https://")).optional().describe("URLs (website, social profile, etc.)"),
  },
  { readOnlyHint: false },
  async (input) => {
    try {
...
server.tool(
  ...
  "Update an existing contact. Provide the contact's identifier and only the fields you want to change — other fields are left untouched.",
  {
    ...
    birthday: z.string().regex(/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/, "Birthday must be in YYYY-MM-DD format").refine((v) => { try { const d = new Date(v + "T00:00:00"); return d.toISOString().startsWith(v); } catch { return false; } }, { message: "Birthday is not a valid calendar date" }).optional().describe("New birthday in YYYY-MM-DD format"),
    phoneNumbers: z.array(z.string().min(1).regex(/^\+\d{2,}$/, "Phone number must be in E.164 format (e.g. +14155551234)")).optional().describe("Replace phone numbers"),
    emailAddresses: z.array(z.string().min(1).regex(/^[^\s@]+@[^\s@]+\.[^\s@]+$/, "Invalid email address format")).optional().describe("Replace email addresses"),
    urlAddresses: z.array(z.string().min(1).regex(/^https?:\/\/\S+$/, "URL must start with http:// or https://")).optional().describe("Replace URLs"),
  },
  { readOnlyHint: false },
